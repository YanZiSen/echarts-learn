...
Store.prototype.get = async function (id) {
    let data = null, getChain = this.getChain.slice()
...
Store.prototype.set = async function (id, data) {
    let isSet = false, setChain = this.setChain.slice()
    while (!isSet) {
...
var dataReduce = [
    ...
    async function getFromRemote (id) {
        let {code,data} = await send(id)
        return data
    },
    async function setRemote (id, data) {
        let {code, data} = await send({id, data})
        return false
    },
    async function setCache (id, data) {
        if (data.isHot) { // 如果是热门商品，则在缓存里增加
            this.cache[id] = data
        }
        return this.cache[id]
    },
